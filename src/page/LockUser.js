...
class LockUser extends React.Component {
    ...
    lockUser = (values) => {
        ...
        fetch(api + '/user_freeze_by_banker', {
            body: data,
            method: "POST",
            credentials: 'include',
            mode: "cors"
    ...
    render(){
        const form = ()=>{
            if (this.state.status === 1){
                return <PersonalUserForm handle={this.lockUser} isSubmit={true} button={'Lock'} values={this.state.user} isChange={false} />
            }
            else if (this.state.status === 2){
                return <LegalUserForm handle={this.lockUser} isSubmit={true} button={'Lock'} values={this.state.user} isChange={false} />
            }
        }
        return (
            <div>
